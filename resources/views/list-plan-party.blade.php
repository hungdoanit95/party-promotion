          ...
          <div class="card">
            <div class="card-body">
              ...
              <div class="row">
                <div class="col-sm-3">
                  <select id="route-plan-ip" name="route_plan" class="form-select form-control" style="margin-top: 20px">
                    ...
                    @foreach($route_plans as $route)
                      <option {{ (isset($params_search['route_plan']) && $params_search['route_plan'] == $route )?"selected":""; }} value="{{ $route }}">{{ $route }}</option>
                    @endforeach            
                  </select>
                </div>  
              </div>
              ...
              <table class="table" style="min-width: 900px">
                    <thead>
                    <tr>
                        <th width="150" scope="col">Hình</th>
                        <th width="190" scope="col">Chủ tiệc</th>
                        <th width="305" scope="col">Thông tin</th>
                        <th width="180" scope="col">NV thực hiện</th>
                        <th width="235" scope="col">Ghi chú</th>
                        <th width="55" scope="col"></th>
                    </tr>
                    </thead>
                    <tbody>
                        @foreach($plan_parties as $key => $plan)
                          @if(!empty($plan))
                            <tr id="row-plan-<?= $plan->plan_party_id; ?>" style="font-size: 14px; position:relative;overflow: hidden;">
                                <td scope="row" style="position:relative; text-align:center;">
                                  <div class="group-button">
                                    @if(!empty($user_logged) && in_array($user_logged['group_id'],array(1,2,3,4,5)))
                                      <button class="btn btn-danger" id="btn-delete-plan" type="button" onclick="delPlan(<?= $plan->plan_party_id; ?>)"><i class="bi bi-trash3"></i></button>
                                    @endif
                                  </div>  
                                  <a href="{{ !empty($plan_image_dt[$plan->plan_party_id])?'/storage/app/'.$plan_image_dt[$plan->plan_party_id]:asset('assets/img/store.jpg')}}" class="td-image" target="_blank"><img width="120" style="border-radius: 5px; max-height: 100px; object-fit: cover;" src="{{ !empty($plan_image_dt[$plan->plan_party_id])?'/storage/app/'.$plan_image_dt[$plan->plan_party_id]:asset('assets/img/store.jpg')}}" /></a>
                                  <p style="margin-bottom: 2px; font-size: 13px; text-align:center; ">{{ $plan->party_code }}</p>
                                </td>
                                <td>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">{{ $plan->party_host_name }}</p>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">{{ $plan->party_host_phone }}</p>
                                </td>
                                <td>
                                  <p style="margin-bottom: 2px; font-size: 13px; ">Loại tiệc: {{ $plan->party_type }}</p>  
                                  <p style="margin-bottom: 2px; font-size: 13px; ">Vào lúc: {{ $plan->organization_date }} {{ $plan->organization_time }}</p>  
                                  <p style="margin-bottom: 2px; font-size: 13px; ">Tại: {{ $plan->home_number }} {{ $plan->street }} {{ $plan->ward }} {{ $plan->district }} {{ $plan->province }}</p>  
                                  <p style="margin-bottom: 2px; font-size: 13px; ">Loại tiệc: {{ $plan->party_type }}</p>  
                                </td>
                                <td>
                                  <p style="margin-bottom: 2px; font-size: 13px; ">{{ $plan->username }}</p>
                                  <p style="margin-bottom: 2px; font-size: 13px; ">{{ $plan->telephone }}</p>
                                </td>
                                <td>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">{{ $plan->notes }}</p>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">NPP: {{ $plan->distributor }}</p>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">Tên NPP: {{ $plan->point_of_salename }}</p>
                                    <p style="margin-bottom: 2px; font-size: 13px; ">SDT NPP: {{ $plan->point_of_salephone }}</p>
                                </td>
                                <td>
                                    <a href="{{ url('/get-info-plan/'.$plan->plan_party_id) }}" target="_blank"><i class="bx bxs-edit" style="font-size: 25px; margin-top: 20px;"></i></a>
                                </td>
                            </tr>
                          @endif
                        @endforeach
